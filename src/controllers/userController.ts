...
export const getUsers = async (c: Context) => {
    try {
        ...
        // optional: balikan 500 supaya konsisten
...
export const updateUser = async (c: Context) => {
    try {
        ...
        //ekstrak data tervalidasi (opsional fields)
        const { name, username, email, password } = c.get('validatedBody') as UserUpdateRequest;

        //cek duplikat email/username selain dirinya sendiri
        const existing = await prisma.user.findFirst({
            where: {
                OR: [{ email }, { username }],
        ...
        //update user (hash password hanya jika dikirim)
